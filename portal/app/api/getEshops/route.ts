...
export async function POST(request: Request) {
  console.log("API route /api/getEshops called");
  ...
  try {
    ...
    console.log("Response from getEshops function:", responseData);
    if (!response.ok) {
      console.error("Failed to fetch eshops:", responseData);
      return NextResponse.json({ message: 'Failed to fetch eshops', error: responseData.error }, { status: response.status });
    }
    return NextResponse.json(responseData, { status: 200 });
  } catch (error: any) {
    console.error('Error verifying token or fetching eshops:', error);
